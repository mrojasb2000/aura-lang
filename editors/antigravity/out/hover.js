...
exports.AuraHoverProvider = exports.HOVER_DATABASE = void 0;
exports.createMarkdownHover = createMarkdownHover;
const vscode = require("vscode");
exports.HOVER_DATABASE = {
    // ------------------------------------------------------------------------
    // Keywords
    // ------------------------------------------------------------------------
    'fn': {
    ...
    'embed': {
        ...
        description: 'Embeds a static text asset directly into the standalone binary output at compile time as a UTF-8 `String`.',
        params: [
...
function createMarkdownHover(doc) {
...
class AuraHoverProvider {
    provideHover(document, position, _token) {
        // Regex to capture identifiers, keywords, decorator (@test), and operators (->, <-, |>, =>, ?)
        const range = document.getWordRangeAtPosition(position, /@[a-zA-Z_][a-zA-Z0-9_]*|\b[a-zA-Z_][a-zA-Z0-9_]*\b|\|>|<-|->|=>|\?/);
        if (!range) {
            return undefined;
        }
        const word = document.getText(range);
        const doc = exports.HOVER_DATABASE[word];
        if (doc) {
...
exports.AuraHoverProvider = AuraHoverProvider;
